...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			add.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					if (add.getText().equals("Edit Comment")) {
						list.setItems(model.editComment(commentItem, fromTime.getText(), toTime.getText(),
								comments.getText(), list));
				...
				public void handle(ActionEvent event) {
					...
					if (file != null) {
						try {
							...
							model.showMessage(
									"Error: There was a problem with creating the .doc file. Please try again.");
						}
					}
				}
			});
			...
			// displays the Menu Bar "Navigate" option with all options and
			// hotkeys
			...
			openVideo.setOnAction(event -> {// when open is selected, gets info
											// from selected csv file and
				...
				if (file != null) {
					...
					try {
						...
						model.showMessage("Error: There was a problem with this video file. "
								+ "Are you sure it's a video file? Are you sure this file was encrypted correctly?");
						e.printStackTrace();
					}
				}
			});

			openReview.setOnAction(event -> {// when open is selected, gets info
												// from selected csv file and
												// enables test options
				FileChooser fileChooser = new FileChooser();
				...
				if (file != null) {
					try {
						...
						model.showMessage(
								"Error: We had trouble opening the file. Make sure that it's in the proper format and make sure that the "
			...
			analyze.setOnAction(event -> {// change view to show the analysis
											// screen
				root.setRight(grid);
				analyze.setDisable(true);
				comment.setDisable(false);
			...
			comment.setOnAction(event -> {// change view to show the analysis
											// screen
				root.setRight(grid2);
				analyze.setDisable(false);
				comment.setDisable(true);
